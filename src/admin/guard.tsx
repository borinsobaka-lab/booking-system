 ...
 * администратор вдобавок правит расписание и работает с записями.
 */
  ...
  const node = open ? (
    <Modal title="Только просмотр" onClose={() => setOpen(false)}>
      <div className="form">
        <p className="muted">
          {isAdmin
            ? 'Ваша роль — администратор: вы редактируете расписание, отменяете записи и ставите отметки. Остальные разделы доступны только для просмотра — изменить их может суперадминистратор.'
