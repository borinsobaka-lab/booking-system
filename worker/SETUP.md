...
| POST | `/api/bookings/create` | владелец (токен) | создать запись из админки |
| POST | `/api/bookings/cancel` | владелец (токен) | отменить запись |
...
специалистов), от сотрудника — отвечает 403.
