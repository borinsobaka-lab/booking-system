...
> заводится **вручную** (шаг 2), администраторов и сотрудников создаёт уже он
> внутри панели.
...
- Каждое изменение данных — отдельный коммит в приватный репозиторий, и
  **подпись коммита написана по-человечески**: «Новая запись: Ана → Нино Ц. ·
  14 сентября, 10:00 · Массаж», «Отмена записи: …», «Новый абонемент: … ·
  8 посещений». Если к приватному репозиторию подключён бот (например, GitHub
  в Telegram), эти подписи и есть уведомления о записях, отменах и абонементах.
  Телефоны и почты в подписи не попадают — только имя клиента, мастер, время и
  услуга. Тексты собираются в `bookingNote`/`membershipNote` (`src/logic.js`).
...
| POST | `/api/bookings/create` | владелец / администратор (токен) | создать запись из админки |
| POST | `/api/bookings/cancel` | владелец / администратор (токен) | отменить запись |
| POST | `/api/bookings/pay` | владелец / администратор (токен) | отметка об оплате сеанса массажисту |
| POST | `/api/bookings/membership` | владелец / администратор (токен) | отметка «по абонементу» |
| POST | `/api/auth/login` | сотрудник | вход, выдаёт токен сессии |
| GET  | `/api/data` | сотрудник (токен) | полные данные для админки |
| PUT  | `/api/data` | владелец / администратор (токен) | сохранить данные: владелец — всё (кроме записей, они меняются эндпоинтами выше), администратор — только расписание |

Права проверяет сервер, а не браузер: от администратора `PUT /api/data`
принимает только поле `schedules` (и только корректные дни существующих
специалистов), от сотрудника — отвечает 403. С записями администратор работает
наравне с владельцем (создание, отмена, отметки); приглашения клиентам и
изменение остальных данных остаются за владельцем.
...
В `wrangler.toml` заданы `EMAIL_FROM` (`NEBA <noreply@neba.space>` — домен
подтверждён), `STUDIO_TZ` (`Asia/Tbilisi`) и cron `*/10 * * * *`. Домен neba.space
верифицирован в Resend, поэтому письма уходят реальным клиентам и мастерам —
секрет `TEST_EMAIL` для боевого режима нужно удалить.
